...
No neural network, no learned parameters, nothing. Just the compressor that ships with your operating system. You prime it with a corpus, and it continues a prompt by searching for the byte sequences that compress best. Here's some real, unedited output after priming it on tiny Shakespeare:
...
The fix is to look *ahead* a whole span before committing. `gzipt` runs a [beam search](https://en.wikipedia.org/wiki/Beam_search) over byte sequences:
...
[^1]: The paper did try this, but it ended up performing really poorly. Adding beam search significantly improved generation quality, which is discussed below.
