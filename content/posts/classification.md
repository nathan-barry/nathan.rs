...
These models, on the other hand, focus on modeling the conditional probability $ p(y|x) $. They directly compute:
...
## Models
Classification consists of assigning a predefined label to a new, unseen data point based on the learning from the training data. Below are four classic types of classifiers:
...
The following sections contain more in depth details on each model.
...
The Perceptron is one of the simplest types of artificial neural networks and a form of binary classifier. The algorithm starts with a weight vector initialized to zero and iteratively updates it using individual training examples.
