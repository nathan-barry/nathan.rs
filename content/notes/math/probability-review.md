...
The variance can be calculated as: {{%sidenote%}}$(1-p)$ is sometimes denoted as $q$.{{%/sidenote%}}
$$Var[X]=p\cdot(1-p)$$
...
3. The variance can be calculated as:
...
6. *The Memoryless Property:* Let \\(X\\) be an exponential random variable with mean \\(\beta\\). We have:
...
$$Pr(-.2<X<6.9)=Pr(-2.6<Z<.95)$$
$$=\Phi(.95)-[1-\Phi(2.6)]=.8289+.9953-1=.8243$$
...
Let $Y=e^X$ where $X\sim N(\mu_X, \sigma_X^2)$. Then $Y$ is said to have a lognormal distribution with parameters $\mu_X$ and $\sigma_X^2$.

1. Expected Value: $$E[Y]=e^{\mu_X+(1/2)\sigma_X^2}$$
2. Variance: $$Var[Y]=e^{2\mu_X+\sigma_X^2}\cdot(e^{\sigma_X^2}-1)$$
3. The [[Cumulative Distribution Function]] for $Y$ is $$F_Y(y)=\Phi\bigg(\frac{\ln y-\mu_X}{\sigma_X}\bigg)\qquad 0<y<\infty$$
4. The [[Density Function]] for $Y$ is $$f_Y(y)=\frac{1}{y\sigma_X\sqrt{2\pi}}e^{-\frac12(\frac{\ln y-\mu_X}{\sigma_X})^2}$$
5. For $0<a<b$, $$Pr[a<Y\le b ]=\Phi\bigg(\frac{\ln b-\mu_X}{\sigma_X}\bigg)-\Phi\bigg(\frac{\ln a-\mu_X}{\sigma_X}\bigg)$$

### Important Integrations

Helpful with a normal distribution:
$$\int_{-\infty}^{\infty}e^{-(z^2/2)} dz=\sqrt{2\pi}$$

Helpful with a exponential distribution.Let $b>0$ be a positive constant. Then:
$$\int_0^\infty e^{-bx}dx=\frac1b$$
...
### Moment Generation Functions for Discrete Distributions 
...
A geometric distribution is a series of independent Bernoulli Trials with probability of success $p$ where $X$ denotes the number of failures prior to the first success. 

$$p_k=Pr[X=x]=p\cdot(1-p)^k\quad for\quad k=0,1,2,\dots,\infty$$
...
Recall that the negative binomial distribution is the generalization of the geometric random variable. In a series of independent Bernoulli Trials, we let $S$ denote the number of failures prior to the $r^{th}$ success. Then $S$ is a negative binomial random variable and can be expressed as $S=X_1+\cdots+X_r$, where the $X_i$'s are independent and have identical geometric distributions.

Let $S$ be the number of failures before the $r^{th}$ success in sequence of independent identical Bernoulli Trials with probability of success $p$. Then $S$ is a negative binomial distribution with the MGF given by:
...
Let $X$ be a Poisson random variable with a mean rate of occurrence $\lambda$. We know that the probability function of $X$ is $p_k=Pr[X=k]=e^{-\lambda}\frac{\lambda^k}{k!}$, for $k=0,1,2,\dots$. Then the MGF for $X$ is given by 
